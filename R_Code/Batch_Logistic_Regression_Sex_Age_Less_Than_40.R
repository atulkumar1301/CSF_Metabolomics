...
df <- fread (file = "/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_1/Age_Stratified/Age_Stratified_Less_Than_40/1_Data_Age_Stratified_Less_Than_40.txt")
df_2 <- df [,1:29]
j <- 1
for (i in colnames (df)) {
  ...
  N_P <- df[[i]]
  model <- glm (Gender ~ N_P, data = df, family = binomial (link = 'logit'))
...
write.table (TABLE, (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_1/Age_Stratified/Age_Stratified_Less_Than_40/2_Result_Data_Raw_Analysis.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
