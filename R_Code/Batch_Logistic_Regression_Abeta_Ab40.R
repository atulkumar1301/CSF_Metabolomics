...
for (i in colnames (df)) {
  ...
  N_P <- df[[i]]
  model <- glm (Abnormal_CSF_Ab42_Ab40_Ratio ~ N_P + Age + Gender + CSF_Ab40_ngml_imputed_Elecsys_2020_2022 + Medication, data = df, family = binomial (link = 'logit'))
...
write.table (TABLE, (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/A_Beta/Ab40/6_Result_Data_Analysis_A_beta_Ab40.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
