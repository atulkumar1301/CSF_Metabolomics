...
for (i in colnames (df)) {
  ...
  N_P <- df[[i]]
  model <- glm (tnic_cho_com_I_IV ~ N_P + Age + Gender + Recruitment_Bias + CSF_Ab40_ngml_imputed_Elecsys_2020_2022, data = df, family=gaussian)
...
write.table (TABLE, (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/TauPET/Ab40/6_Result_Data_Analysis_Taupet_Ab40.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
