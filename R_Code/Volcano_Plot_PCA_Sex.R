...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_1/PCA/14_Full_Result_Data_Analysis_PCA.txt")
...
p <- p + scale_x_continuous(breaks = round(seq(-0.5, 0.25, by = 0.1),1))
p <- p + scale_y_continuous(breaks = round (seq (0, 17, by = 1), 1))
...
p <- p +
  theme(legend.position="left",
        ...
        panel.background = element_blank()) + labs(title=expression("Association of Gender with PCs"))
p
