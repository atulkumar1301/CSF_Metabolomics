...
for (i in colnames (df)) {
  ...
  N_P <- df[[i]]
  model <- glm (Abnormal_CSF_Ab42_Ab40_Ratio ~ N_P + Age + Gender + Recruitment_Bias + Medication, data = df, family = binomial (link = 'logit'))
...
write.table (TABLE, (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/A_Beta/Medication/6_Result_Data_Analysis_A_beta_Medication.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
