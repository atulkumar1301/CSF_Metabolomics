...
for (i in colnames (df)) {
  ...
  model <- glm (N_P ~ tnic_cho_com_I_IV + Age + Gender + Recruitment_Bias + mean_standardized_metabolomic_level + AD_PD_Medication, data = df, family=gaussian)
...
write.table (TABLE, (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/Metabolites/TauPET/Medication/6_Result_Data_Analysis_Taupet_Medication.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
