...
for (i in colnames (df)) {
  ...
  model <- glm (N_P ~ SAA_Status + Age + Gender + Recruitment_Bias + mean_standardized_metabolomic_level + AD_PD_Medication, data = df, family=gaussian)
...
write.table (TABLE, (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/Metabolites/A_Syn/Medication/6_Result_Data_Analysis_A_Syn_Medication.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
