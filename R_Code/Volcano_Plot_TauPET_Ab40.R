...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/TauPET/Ab40/7_Full_Result_Data_Analysis_Taupet_Ab40.txt")
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.0017), linetype = "FDR p-value 0.05", col="#D55E00")) +
...
p <- p + scale_x_continuous(breaks = round(seq(-0.25, 0.35, by = 0.05),1))
...
p <- p +
  theme(legend.position="left",
        ...
        panel.background = element_blank()) + labs(title=expression("Differential Regulation of CSF Metabolites with respect to TauPET adjusted for A"*beta*"40"))
p
