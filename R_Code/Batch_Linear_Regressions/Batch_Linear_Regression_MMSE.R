...
for (i in colnames (df)) {
  ...
  model <- glm (N_P ~ mmse_score + Age + Gender + Recruitment_Bias + mean_standardized_metabolomic_level, data = df, family=gaussian)
...
write.table (TABLE, (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/MMSE/6_Result_Data_Analysis_MMSE.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
