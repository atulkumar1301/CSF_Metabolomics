...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/A_Beta/Ab40/8_Full_Result_Data_Analysis_A_beta_Ab40.txt")
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.0023), linetype = "FDR p-value 0.05", col="#D55E00")) +
...
p <- p + scale_y_continuous(breaks = round (seq (0, 7, by = 0.5), 1))
...
p <- p +
  theme(legend.position="left",
        ...
        panel.background = element_blank()) + labs(title=expression("Differential Regulation of CSF Metabolites with respect to A"*beta*" Status adjusted for A"*beta*"40"))
p
