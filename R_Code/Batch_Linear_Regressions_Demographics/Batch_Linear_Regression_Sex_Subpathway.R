...
df <- fread (file = "~/Library/CloudStorage/OneDrive-UniversityofEasternFinland/Work/Projects/CSF_Metabolomics/Analyses_2/Sub-Pathway/Demographics/CUI_Data_Subpathway.txt")
df_2 <- df [,1:28]
j <- 1
for (i in colnames (df)) {
  ...
  model <- glm (N_P ~ Gender + mean_standardized_metabolomic_level, data = df, family=gaussian)
...
write.table (TABLE, (file = paste0 ("~/Library/CloudStorage/OneDrive-UniversityofEasternFinland/Work/Projects/CSF_Metabolomics/Analyses_2/Sub-Pathway/Demographics/6_Result_Data_Analysis_Subpathway_Gender.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
