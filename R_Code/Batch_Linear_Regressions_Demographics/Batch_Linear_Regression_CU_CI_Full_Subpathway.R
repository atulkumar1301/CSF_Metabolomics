...
for (i in colnames (df)) {
  ...
  model <- glm (N_P ~ Recruitment_Bias + mean_standardized_metabolomic_level, data = df, family=gaussian)
...
write.table (TABLE, (file = paste0 ("~/Library/CloudStorage/OneDrive-UniversityofEasternFinland/Work/Projects/CSF_Metabolomics/Analyses_2/Sub-Pathway/Demographics/Full/6_Result_Data_Analysis_Subpathway_CU_CI_Full.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
