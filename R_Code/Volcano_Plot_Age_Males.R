...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_1/Sex_Stratified/Male/8_Full_Result_Data_Analysis_Age_Male.txt")
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.039), linetype = "FDR p-value 0.05", col="#D55E00")) +
...
p <- p + scale_x_continuous(breaks = round(seq(-15, 60, by = 5),1))
p <- p + scale_y_continuous(breaks = round (seq (0, 26, by = 3), 1))
...
p <- p +
  theme(legend.position="left",
        ...
        panel.background = element_blank()) + labs(title="Differential Regulation of CSF Metabolites with respect to Age:Males")
p
