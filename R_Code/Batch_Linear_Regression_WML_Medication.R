...
for (i in colnames (df)) {
  ...
  N_P <- df[[i]]
  model <- glm (DV ~ N_P + Age + Gender + Recruitment_Bias + icv_mm3 + Medication, data = df, family=gaussian)
...
write.table (TABLE, (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/WML/Medication/6_Result_Data_Analysis_WML_Medication.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
