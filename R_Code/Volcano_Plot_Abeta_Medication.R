...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/A_Beta/Medication/8_Full_Result_Data_Analysis_A_beta_Medication.txt")
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.00163), linetype = "FDR p-value 0.05", col="#D55E00")) +
...
p <- p + scale_x_continuous(breaks = round(seq(-2, 2, by = 0.5),1))
p <- p + scale_y_continuous(breaks = round (seq (0, 8, by = 1), 1))
...
p <- p +
  theme(legend.position="left",
        ...
        panel.background = element_blank()) + labs(title=expression("Differential Regulation of CSF Metabolites with respect to A"*beta*" Status adjusted for medication"))
p
