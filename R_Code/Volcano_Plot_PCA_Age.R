...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_1/PCA/14_Full_Result_Data_Analysis_Age_Raw_Adjusted.txt")
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.0031), linetype = "FDR p-value 0.05", col="#D55E00")) +
...
p <- p + scale_x_continuous(breaks = round(seq(-10, 10, by = 1),1))
p <- p + scale_y_continuous(breaks = round (seq (0, 45, by = 3), 1))
p <- p + xlab ("Effect") + labs (color = "Regulation") + ylab (expression (-log[10]~(P)))
p <- p +
  theme(legend.position="left",
        plot.title = element_text(family = "serif", size=17, face = "bold", hjust = 0.5),
        ...
        panel.background = element_blank()) + labs(title=expression("Association of Age with PCs"))
p
