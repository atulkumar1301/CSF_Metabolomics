...
Nodes <- fread ("A-beta_Nodes.txt")
Edges <- fread ("A-beta_Edges.txt")
...
plot(net.i, vertex.label=V(net.i)$Nodes, layout = l.i, vertex.label.family = "Times", vertex.label.dist	= -pi/2, vertex.label.color="black", vertex.label.font	= 4,
     main = expression ("Increased Metabolites associated with A"*beta* " with and without adjustment"))

####Decreased Metabolites
Nodes <- fread ("A-beta_Nodes_Negative.txt")
Edges <- fread ("A-beta_Edges_Negative.txt")
...
plot(net.d, vertex.label=V(net.d)$Nodes, layout = l.d, vertex.label.family = "Times", vertex.label.dist	= -pi/2, vertex.label.color="black", vertex.label.font	= 4,
     main = expression ("Decreased Metabolites associated with A"*beta* " with and without adjustment"))
