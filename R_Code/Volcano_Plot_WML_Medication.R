...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/WML/Medication/7_Full_Result_Data_Analysis_WML_Medcation.txt")
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.007), linetype = "FDR p-value 0.05", col="#D55E00")) +
...
#p <- p + scale_x_continuous(breaks = round(seq(-0.1, 0.05, by = 0.005),1))
...
p <- p +
  theme(legend.position="left",
        ...
        panel.background = element_blank()) + labs(title=expression("Differential Regulation of CSF Metabolites with respect to WML adjusted for Medication"))
p
