...
for (i in colnames (df)) {
  ...
  N_P <- df[[i]]
  model <- glm (tnic_cho_com_I_IV ~ N_P + Age + Gender + Recruitment_Bias + Medication, data = df, family=gaussian)
...
write.table (TABLE, (file = paste0 ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/TauPET/Medication/6_Result_Data_Analysis_Taupet_Medication.txt")), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
