...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_1/8_Full_Result_Data_Analysis_Age_Raw_Adjusted.txt")
...
p <- p + scale_x_continuous(breaks = round(seq(-6.6, 11.8, by = 2),1))
p <- p + scale_y_continuous(breaks = round (seq (0, 95, by = 5), 1))
...
p <- p +
  theme(legend.position="left",
               ...
               panel.background = element_blank()) + labs(title="Differential Regulation of CSF Metabolites with respect to Age \n Adjusted for Sex")
p
