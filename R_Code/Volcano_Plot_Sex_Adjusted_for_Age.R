...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_1/8_Full_Result_Data_Analysis_Sex_Raw_Adjusted.txt")
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.023), linetype = "FDR p-value 0.05", col="#D55E00")) +
...
p <- p + scale_x_continuous(breaks = round(seq(-5.6, 4.5, by = 1),1))
p <- p + scale_y_continuous(breaks = round (seq (0, 22, by = 2), 1))
