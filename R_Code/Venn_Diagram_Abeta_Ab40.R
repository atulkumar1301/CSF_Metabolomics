...
df_abeta_Ab40 <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/A_Beta/Ab40/8_Full_Result_Data_Analysis_A_beta_Ab40.txt")
df_abeta_Ab40_Sig_Pos <- df_abeta_Ab40[df_abeta_Ab40$P_FDR <= 0.05 & df_abeta_Ab40$Effect > 0]
df_abeta_Ab40_Sig_Neg <- df_abeta_Ab40[df_abeta_Ab40$P_FDR <= 0.05 & df_abeta_Ab40$Effect < 0]
abeta_Ab40_Sig_Pos <- df_abeta_Ab40_Sig_Pos$Protein
abeta_Ab40_Sig_Neg <- df_abeta_Ab40_Sig_Neg$Protein

## Positive
venn_Pos <- list (abeta_Sig_Pos, abeta_Medication_Sig_Pos, abeta_Ab40_Sig_Pos)
p <- ggVennDiagram (venn_Pos, label_alpha = 0,category.names = c("Abeta", "Medication", "Ab40"))
p <- p + ggplot2::scale_fill_gradient(low="#D55E00",high = "#009E73") +
  ggtitle (expression ("Increased level of Metabolites in A"*beta*" Status, A"*beta*" Status adjusted for Medication and A"*beta*" Status adjusted for A"*beta*"40"))
p <- p + theme(
...
## Negative
venn_Neg <- list (abeta_Sig_Neg, abeta_Medication_Sig_Neg, abeta_Ab40_Sig_Neg)
p1 <- ggVennDiagram (venn_Neg, label_alpha = 0,category.names = c("Abeta", "Medication", "Ab40"))
p1 <- p1 + ggplot2::scale_fill_gradient(low="#E69F00",high = "#56B4E9") +
  ggtitle (expression ("Decreased level of Metabolites in A"*beta*" Status, A"*beta*" Status adjusted for Medication and A"*beta*" Status adjusted for A"*beta*"40"))
p1 <- p1 + theme(
  plot.title = element_text(family = "serif", size=18, face = "bold"),
  axis.title.x = element_blank(),
  axis.title.y = element_blank(),
