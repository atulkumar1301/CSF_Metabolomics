...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/CSF_Metabolomics/Analyses_2/Sub-Pathway/Heat_Map_Data_Full.txt")
melted_df <- melt (df)
ggheatmap <- ggplot (data = melted_df, aes (x = Metabolite, y = variable, fill = value)) + 
          geom_tile (color = "white") +
  scale_fill_gradient2(low = "blue", high = "red", midpoint = -1.0061072, limit = c(-5, 6), space = "Lab", name="Mediation Percentage")+
  xlab("Metabolites") +
  ylab("Pathologies") +
